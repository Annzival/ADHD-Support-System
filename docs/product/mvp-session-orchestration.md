...
flowchart TD
    ...
    B02 --> C1{"恢复干预进入首个闭环？"}
    B02 --> C2{"LLM 阻碍支持进入首个闭环？"}
    B02 --> C3{"允许未决执行上下文并存？"}
    C1 -->|是| V05["V-05 恢复干预载体原型"]
    C2 -->|是| V06["V-06 缩小步骤 prototype / eval"]
    C3 -->|是| VX["上下文竞争交互原型"]
    B03 --> V07["V-07 模型供应商切换与不可用边界"]
    V06 --> V07
...
条件节点为“否”时，对应能力按构建就绪文档重新归入实现中再决定或 MVP 后，不再阻塞首个闭环。V-07 只有在首个闭环包含 LLM 时才适用，并同时等待 V-06 和 B-03；它验证 PydanticAI 模型供应商切换、模型不可用回退和权威状态独立性。图中的连线表达最晚依赖；不适用的条件分支不需要创建 Issue 或 session。
