...
已核验 2026-09-22 回传包：A、B、D 通过各自范围；C 的时长确认通过，但小窗／主窗口原生关闭失败。已补充关闭路径修复，整体仍等待 C 轮复测，不报告 I-01 PASS。详见[证据核验与修复记录](results/mvp-first-vertical-slice.md#windows-首轮证据核验与原生关闭修复)。
...
## 本次只复测 C 轮（已回传 A～D 的操作者）

保留旧 C 轮目录和 FAIL 记录，不修改或覆盖。A、B、D 的原证据继续有效，无须重做；它们对应原 commit，修复后的 C 轮会有独立 commit／二进制身份。

1. 从托盘退出旧应用，按 0.1 更新代码并确认干净；按 0.2 恢复 WebView2 和 Python 路径。
2. 执行下面命令，建立**新的**复测批次，记下输出目录：

```powershell
$batchDir = Join-Path (Get-Location).Path ('.i01-runs\C-retest-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
New-Item -ItemType Directory -Path $batchDir | Out-Null
$runC = Join-Path $batchDir 'C'
$batchDir
```

3. 只执行本文 **C1～C6**。重点观察 C4 的小窗叉号是否隐藏并可重开、C5 的主窗口叉号是否保存跳过收尾并隐藏。仍失败就如实填写 FAIL。
4. C6 Collect 成功后，使用“最终回传什么”的打包命令，**第一行改为 `$completedRuns = @($runC)`**，其余照抄。
5. 上传新的 ZIP，说明“C 轮复测”、两个关闭步骤的实际现象和 ZIP SHA-256。不用重复上传原 A／B／D。
