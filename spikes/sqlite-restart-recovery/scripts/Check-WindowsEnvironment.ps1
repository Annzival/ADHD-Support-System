...
if ($py) {
    $pythonProbeCode = 'import json, struct, sys; print(json.dumps({"version":".".join(map(str, sys.version_info[:3])),"processBits":struct.calcsize("P") * 8}))'
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        try {
            ...
            $report.actual.python = [ordered]@{ version = $pythonVersion; processBits = $pythonProcessBits }
