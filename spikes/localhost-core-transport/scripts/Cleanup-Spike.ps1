...
$core = Join-Path $root 'agent_core\agent_core_stub.py'
...
foreach ($process in @(Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like "*$core*" })) {
    Stop-Process -Id $process.ProcessId -Force
    Write-Host "Stopped Core test double PID $($process.ProcessId)"
}

Write-Host 'Cleanup completed. Evidence directories were preserved.'
